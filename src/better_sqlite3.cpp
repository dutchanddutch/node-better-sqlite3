...
#line 4 "./src/objects/database.lzz"
                ...
                SetPrototypeMethod(isolate, data, t, "getTransactionState", JS_getTransactionState);
...
void Database::JS_getTransactionState (v8::FunctionCallbackInfo <v8 :: Value> const & info)
#line 416 "./src/objects/database.lzz"
                                            {
                Database* db = node :: ObjectWrap :: Unwrap <Database>(info.This());
                int state;
                if (info.Length() == 0 || info[0]->IsUndefined()) {
                        state = sqlite3_txn_state(db->db_handle, NULL);
                } else {
                        if ( info . Length ( ) <= ( 0 ) || ! info [ 0 ] -> IsString ( ) ) return ThrowTypeError ( "Expected " "first" " argument to be " "a string" ) ; v8 :: Local < v8 :: String > schema = ( info [ 0 ] . As < v8 :: String > ( ) ) ;
                        v8 :: Isolate * isolate = info . GetIsolate ( ) ;
                        state = sqlite3_txn_state(db->db_handle, *v8::String::Utf8Value(isolate, schema));
                        if (state < 0) {
                                ThrowRangeError("Invalid schema name");
                                return;
                        }
                }
